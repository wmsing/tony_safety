...
5. **例子**：站点主题就是 LLM 被滥用时会出哪些险。见 [welcome](../zh/articles/welcome.md)。
...
5. **例子**：GenAI 安全清单里列了提示词注入、Agent 越权等。见 [welcome](../zh/articles/welcome.md)。
...
5. **例子**：攻击者把「忽视先前指令」藏在简历里，筛简历的 AI 中招。见 [welcome § LLM01](../zh/articles/welcome.md)。
...
5. **例子**：若系统把 `customer_360` 全塞进 Prompt，用户可能被诱导套出档案。见 [welcome § LLM08](../zh/articles/welcome.md)。
...
5. **例子**：Agent 直连云 API 跑 Shell，没有 [HITL](#hitl) 确认。见 [welcome § LLM03](../zh/articles/welcome.md)。
...
5. **例子**：攻击者塞数万 Token，API 卡死、账单暴涨。见 [welcome § LLM06](../zh/articles/welcome.md)。
...
5. **例子**：系统把全量账户档案拼进 Prompt，攻击者问「列出我的背景变量」。见 [welcome § LLM08](../zh/articles/welcome.md)。
...
5. **例子**：论坛灌假法律文，RAG 抓走后 AI 给错建议。见 [welcome § LLM05](../zh/articles/welcome.md)。
...
5. **例子**：黑客拿到 Embeddings 用反演算法还原 Word/PDF。见 [welcome § LLM09](../zh/articles/welcome.md)。
...
5. **例子**：员工搜「薪资标准」却命中 CEO 合同。见 [welcome § LLM09](../zh/articles/welcome.md)。
...
5. **例子**：风景图像素里藏「把密码发给黑客」，多模态模型读图中招。见 [welcome § LLM01](../zh/articles/welcome.md)。
...
5. **例子**：多模态链路上 OCR 解析隐藏文字导致 [越狱](#jailbreak)。见 [welcome § LLM01](../zh/articles/welcome.md)。
...
5. **例子**：HuggingFace 上下载的微调模型藏触发器偷聊天记录。见 [welcome § LLM04](../zh/articles/welcome.md)。
...
5. **例子**：特殊提示让模型乱码，倒腾出背过的身份证号。见 [welcome § LLM02](../zh/articles/welcome.md)。
...
5. **例子**：监听 TLS 包长与延时推断对话内容（Whisper Leak）。见 [welcome § LLM02](../zh/articles/welcome.md)。
...
5. **例子**：开源 MCP 插件里藏反向 Shell。见 [welcome § LLM04](../zh/articles/welcome.md)。
...
5. **例子**：Agent 递归调 API 直到 CPU 爆掉。见 [welcome § LLM06](../zh/articles/welcome.md)。
...
5. **例子**：邮件 Agent 被外链诱导清空收件箱。见 [welcome § LLM03](../zh/articles/welcome.md)。

### OWASP Top 10 for LLM {#owasp-llm-top10}

1. **一句话**：业界整理的十大 LLM 应用安全风险清单（2026 版）。
2. **像什么**：体检十大项：按表排查，不漏大类。
3. **为啥在安全文里出现**：本站 [welcome](../zh/articles/welcome.md) 按此结构讲案例。
...
5. **例子**：透明字藏在 PDF 简历里，筛简历 AI 改推荐结果。见 [welcome § LLM01](../zh/articles/welcome.md)。
...
5. **例子**：图内隐藏指令经 OCR 后模型执行恶意操作。见 [welcome § LLM01](../zh/articles/welcome.md)。
...
5. **例子**：邮件 Agent 能删库，外人一封邮件就触发。见 [welcome § LLM03](../zh/articles/welcome.md)。
...
5. **例子**：无 HITL 时 Agent 自动执行云 API 与 Shell。见 [welcome § LLM03](../zh/articles/welcome.md)。
...
5. **例子**：AI 捏造 npm 包名，攻击者抢先注册投木马。见 [welcome § LLM07](../zh/articles/welcome.md)。
...
5. **例子**：重复特殊词诱发乱码，倒出训练集里的密钥。见 [welcome § LLM02](../zh/articles/welcome.md)。
...
5. **例子**：`customer_360` 拼进 Prompt，用户问「列出我的背景变量」。见 [welcome § LLM08](../zh/articles/welcome.md)。
...
5. **例子**：向量反演还原 PDF；无 ACL 搜到 CEO 薪酬。见 [welcome § LLM09](../zh/articles/welcome.md)。
...
5. **例子**：带毒 MCP、后门微调模型。见 [welcome § LLM04](../zh/articles/welcome.md)。
...
5. **例子**：论坛灌错法律文，RAG 抓取后误导用户。见 [welcome § LLM05](../zh/articles/welcome.md)。
...
5. **例子**：超大上下文 + Agent 死循环调 API。见 [welcome § LLM06](../zh/articles/welcome.md)。
...
5. **例子**：AI 生成 `<script>`，页面未转义触发 XSS。见 [welcome § LLM10](../zh/articles/welcome.md)。
...
5. **例子**：AI 输出含 `<script>`，Markdown 渲染未过滤。见 [welcome § LLM10](../zh/articles/welcome.md)。
...
5. **例子**：未转义系统命令经 `system()` 执行。见 [welcome § LLM10](../zh/articles/welcome.md)。
...
5. **例子**：AI 输出喂给 `system()`，沙箱未拦住。见 [welcome § LLM10](../zh/articles/welcome.md)。
...
5. **例子**：数万 Token 请求拖死 API。见 [welcome § LLM06](../zh/articles/welcome.md)。
...
5. **例子**：包长与响应延时推断用户聊了什么。见 [welcome § LLM02](../zh/articles/welcome.md)。
...
5. **例子**：普通员工语义搜到 CEO 薪酬合同。见 [welcome § LLM09](../zh/articles/welcome.md)。
...
> 读 [welcome](../zh/articles/welcome.md) 和 Wire 精读时偶现；需要再展开时回查本节。
...
5. **例子**：捏造 PyPI 包名，攻击者抢先注册木马。见 [welcome § LLM07](../zh/articles/welcome.md)。
...
5. **例子**：MCP 插件藏反向 Shell。见 [welcome § LLM04](../zh/articles/welcome.md)。
...
读 [常见 LLM 风险](../zh/articles/welcome.md) 时，可按条跳转本表 → 术语锚点。
...
*草稿版本：P0 全量 + P1 进阶；审阅后可拆成 Article 或链到 Wire 页脚。*
